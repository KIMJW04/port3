...
const MusicPlayer = ({ isVisible, setVisible }) => {
    ...
    const handleOutsideClick = (event) => {
        if (musicPlayerRef.current && !musicPlayerRef.current.contains(event.target)) {
            setVisible(false);
        }
    };
            ...
            <div id="player" className="relative overflow-visible transition-transform duration-500 transform shadow-lg MusicPlayer__player w-72 h-72 hover:scale-100">
                <YouTube
                    ...
                    opts={{
                        ...
                        height: '0',
                        playerVars: {
                            autoplay: isPlaying ? 1 : 0, // 자동재생은 isPlaying 상태에 따라 다름
                            loop: 0, // 반복 재생
                        },
                    }}
                    onReady={(event) => {
