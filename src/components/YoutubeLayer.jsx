...
const YoutubeLayer = ({ onClose }) => {
    const [activeTab, setActiveTab] = useState("슈카월드");
    const [videos, setVideos] = useState([]);
    const [searchQuery, setSearchQuery] = useState("슈카월드");
    const [loading, setLoading] = useState(false);
    const [loadMoreLoading, setLoadMoreLoading] = useState(false); // 추가: 로드 상태 변수
    ...
    const fetchInitialVideos = async (isLoadMore = false) => {
        if (isLoadMore) {
            setLoadMoreLoading(true); // 추가: 더보기 로딩 상태 설정
        } else {
            setLoading(true);
        }
        ...
        setTimeout(() => {
            if (isLoadMore) {
                setLoadMoreLoading(false); // 추가: 더보기 로딩 상태 해제
            } else {
                setLoading(false);
            }
        }, 1000); // 데이터를 가져온 후 1초 뒤에 로딩 상태를 해제합니다.
    };
