...
const PortLayer = ({ onClose }) => {
    const nodeRef = useRef(null);
    const [selectedItem, setSelectedItem] = useState('project'); // 선택된 항목 상태 추가
    const [selectedDetail, setSelectedDetail] = useState(null); // 상세 항목 상태 추가
    const [history, setHistory] = useState([]); // 사용자가 본 항목들의 히스토리
    const [historyIndex, setHistoryIndex] = useState(-1); // 히스토리 인덱스

    useEffect(() => {
        const layerElement = document.querySelector(".port-layer");
